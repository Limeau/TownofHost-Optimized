        ...
        if (AmongUsClient.Instance.AmHost) // Execute the following only on the Host
        {
            ...
            switch (GameOptionsManager.Instance.CurrentGameOptions.GameMode)
            {
                ...
                case GameModes.HideNSeek:
                    ...
                    if (Options.CurrentGameMode is CustomGameMode.Standard or CustomGameMode.FFA or CustomGameMode.CandR or CustomGameMode.FourCorners or CustomGameMode.UltimateTeam or CustomGameMode.KOTH)
                    {
